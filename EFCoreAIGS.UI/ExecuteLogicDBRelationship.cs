...
public class ExecuteLogicDBRelationship
{
    ...
    private void InitManyToMany()
    {
        ...
        var SpendDetail = new List<SpendingDetails>()
        {
            new SpendingDetails
            {
                SpentOn = "Income && Outcome",
                Amount = 0,
                IncomeSpend = listSpend,
                OutcomeSpend = listSpend
            }
        };

        if (empWithCredit != null) 
        { 
            empWithCredit.SpendingDetails = SpendDetail; 
            repo.Update(empWithCredit); 
            repo.SaveChanges(); 
        }
